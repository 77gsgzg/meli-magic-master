...
// Atomic rate limit via RPC. Fail-closed (external email send).
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const rlRc = await checkRateLimit(supabase, user.id, "reactivation-campaign", 3);
    if (!rlRc.ok) {
      return new Response(JSON.stringify({ error: rlRc.failed ? "Rate limit service unavailable" : "Rate limit exceeded", retry_after_seconds: rlRc.retryAfter }), {
        status: rlRc.failed ? 503 : 429, headers: { ...corsHeaders, "Content-Type": "application/json", ...(rlRc.retryAfter ? { "Retry-After": String(rlRc.retryAfter) } : {}) },
      });
    }
